...
echo "=== 3. 复制启动器 + 服务端 + GUI 文件 ==="
cp start-installer.mjs "$STAGING/"
cp dist-server/server.mjs "$STAGING/"
cp scripts/app.hta "$STAGING/"
cp scripts/launcher.vbs "$STAGING/"
cp scripts/stopper.vbs "$STAGING/"
ls -lh "$STAGING/start-installer.mjs" "$STAGING/server.mjs" "$STAGING/app.hta" "$STAGING/launcher.vbs" "$STAGING/stopper.vbs"
